...
class ApsDown(nn.Module):
    def __init__(self, channels, pad_type='circular', filt_size=3, stride=2, apspool_criterion = 'l2', device = 'cpu'):
...
#             this is the case when polyphase indices are not pre-defined
        ...
        if(self.filt_size==1):
            ...
            return down_func(aps_pad(blurred_inp), self.stride, polyphase_indices, apspool_criterion = self.apspool_criterion, device=self.device)
...
class ApsUp(nn.Module):
    ...
    def forward(self, inp_to_unpool):
        ...
        if(self.filt_size==1):
            ...
            aps_up = up_func(aps_pad(inp), self.stride, polyphase_indices, device=self.device)
...
def aps_downsample_direct(x, stride, polyphase_indices = None, apspool_criterion = 'l2', device='cpu'):
...
def aps_downsample_v2(x, stride, polyphase_indices = None, permute_indices = None, apspool_criterion = 'l2', device='cpu'):
    ...
    if stride==1:
        ...
        if permute_indices is None:
            if device=='cpu':
                permute_indices = permute_polyphase(N).long()
            else:
...
def aps_upsample(x, stride, polyphase_indices, device='cpu'):
    ...
    if stride ==1:
        ...
        if device=='cpu':
            y = torch.zeros(B, 4, C, Nb2**2).to(x.dtype)
        else:
...
def aps_upsample_direct(x, stride, polyphase_indices, device='cpu'):
    ...
    if stride ==1:
        ...
        if device=='cpu':
            y = torch.zeros(B, 4, C,  N1, N2).to(x.dtype)
            y1 = torch.zeros(B, C,  2*N1, 2*N2).to(x.dtype)
        else:
